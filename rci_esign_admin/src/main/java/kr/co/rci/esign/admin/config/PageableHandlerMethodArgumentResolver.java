 ...
 * parsed can be configured. Default configuration uses request parameters
 * beginning with
...
public class PageableHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	static final Pageable DEFAULT_PAGE_REQUEST = new CustomPageable(0, AppConstants.DEFAULT_PAGE_SIZE);
	 ...
	 * {@link PageableDefault} or {@link PageableDefaults} (the latter only
	 * supported in legacy mode) can be found at the method parameter to be
	 * resolved.
	 * <p>
	 * If you set this to {@literal null}, be aware that you controller methods
	 * will get {@literal null} handed into them in case no {@link Pageable}
	 * data can be found in the request. Note, that doing so will require you
	 * supply bot the page <em>and</em> the size parameter with the requests as
	 * there will be no default for any of the parameters available.
	 *
	 * @param fallbackPageable
	 *            the {@link Pageable} to be used as general fallback.
	 */
	...
	/**
	 * Configures the parameter name to be used to find the page number in the
	 * request. Defaults to {@code page}.
	 *
	 * @param pageParameterName
	 *            the parameter name to be used, must not be {@literal null} or
	 *            empty.
	 */
	...
	/**
	 * Retrieves the parameter name to be used to find the page number in the
	 * request. Defaults to {@code page}.
	 *
	...
	/**
	 * Configures the parameter name to be used to find the page size in the
	 * request. Defaults to {@code size}.
	 *
	...
	/**
	 * Retrieves the parameter name to be used to find the page size in the
	 * request. Defaults to {@code size}.
	 *
	 * @return the parameter name to be used, never {@literal null} or empty.
	 */
	...
	/**
	 * Configures a general prefix to be prepended to the page number and page
	 * size parameters. Useful to namespace the property names used in case they
	 * are clashing with ones used by your application. By default, no prefix is
	 * used.
	 *
	 * @param prefix
	 *            the prefix to be used or {@literal null} to reset to the
	 *            default.
	 */
	 ...
	 * and size properties. Defaults to {@code _}. So a qualifier of {@code foo}
	 * will result in a page number parameter of {@code foo_page}.
	 *
	 * @param qualifierDelimiter
	 *            the delimter to be used or {@literal null} to reset to the
	 *            default.
	 */
	 ...
	 * the request parameters. Defaults to {@literal false}, meaning a page
	 * number of 0 in the request equals the first page. If this is set to
	 * {@literal true}, a page number of 1 in the request will be considered the
	 * first page.
	 *
	...
	/**
	 * Indicates whether to expose and assume 1-based page number indexes in the
	 * request parameters. Defaults to {@literal false}, meaning a page number
	 * of 0 in the request equals the first page. If this is set to
	 * {@literal true}, a page number of 1 in the request will be considered the
	 * first page.
	 *
	 * @return whether to assume 1-based page number indexes in the request
	 *         parameters.
	 */
	...
	public boolean supportsParameter(MethodParameter parameter) {
		return CustomPageable.class.equals(parameter.getParameterType());
	}
	...
	public Pageable resolveArgument(MethodParameter methodParameter, ModelAndViewContainer mavContainer,
		...
		return new CustomPageable(page, pageSize, sort);
	}

	/**
	 * Returns the name of the request parameter to find the {@link Pageable}
	 * information in. Inspects the given {@link MethodParameter} for
	 * {@link Qualifier} present and prefixes the given source parameter name
	 * with it.
	 *
	 * @param source
	 *            the basic parameter name.
	 * @param parameter
	 *            the {@link MethodParameter} potentially qualified.
	...
	private static Pageable getDefaultPageRequestFrom(MethodParameter parameter) {
		...
		if (defaults.sort().length == 0) {
			return new CustomPageable(defaultPageNumber, defaultPageSize);
		}

		return new CustomPageable(defaultPageNumber, defaultPageSize, defaults.direction(), defaults.sort());
	}
	...
	/**
	 * Tries to parse the given {@link String} into an integer and applies the
	 * given boundaries. Will return 0 if the {@link String} cannot be parsed.
	 *
	 * @param parameter
	 *            the parameter value.
	 * @param upper
	 *            the upper bound to be applied.
	 * @param shiftIndex
	 *            whether to shift the index if {@link #oneIndexedParameters} is
	 *            set to true.
	 * @return
	 */
