								...
								<div class="boardTop">
									...
									<div class="right">
										...
										<div class="list-search">
											<span class="calArea">
												<span class="calSet"><input type="text" id="schStartDate" name="schStartDate" class="cal" value="${param.schStartDate}" placeholder="2018-03-02"/><button class="btnCal startBtn">달력</button></span>
												 ~
											...
											<span class="selectSet">
												<strong>전체</strong>
												<select>
													<option id="isNoti" value="">전체</option>
													<option id="isNoti" value="1">긴급</option>
													<option id="isNoti" value="0">정기</option>
													<option id="isNoti" value="2">알림</option>
												</select>
											</span>
											<div>
...
<c:forEach var="board" items="${boardList.content}" varStatus="status">
											...
											<tr>
												<td>${board.postNo}</td>
												<td>
													<c:if test="${board.isNoti=='1'}">
														긴급
													</c:if>
													<c:if test="${board.isNoti=='0'}">
														정기
													</c:if>
													<c:if test="${board.isNoti=='2'}">
														알림
													</c:if>
												</td>
												...
												<td>${board.auth}</td>
												<td class="ft11">
												<fmt:parseDate value="${board.startDate}" var="dateFmt" pattern="yyyyMMdd"/>
												<fmt:formatDate value="${dateFmt}" pattern="yyyy-MM-dd"/> ${board.startHh}:${board.startMi}</td>
												<td class="ft11">
												<fmt:parseDate value="${board.endDate}" var="dateFmt" pattern="yyyyMMdd"/>
												<fmt:formatDate value="${dateFmt}" pattern="yyyy-MM-dd"/> ${board.endHh}:${board.endMi}</td>
												</td>
												...
												<!-- 노출여부 팝업 -->
													<span class="exposure frMain">
													<c:if test="${board.popupUseYn=='Y' }">
														<input type="checkbox" checked>
														<i></i>
													</c:if>
													<c:if test="${board.popupUseYn=='N' }">
														<input type="checkbox">
														<i></i>
													</c:if>
													<c:if test="${board.popupUseYn==NULL }">
														<input type="checkbox">
														<i></i>
													</c:if>
								...
								<div class="btnBox">
									<a href="/contents/noticeDetail" class="btnS blue right">신규등록</a>
								</div>
								<!--Paginate-->
